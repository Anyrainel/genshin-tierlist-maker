...
const CharacterPool = ({ characters, onDragStart, className }: CharacterPoolProps) => {
    ...
    <div className={cn('w-full grid grid-cols-7 gap-4', className)}>
      {Object.entries(charactersByElement).map(([element, chars]) => (
        <div key={element} className="flex flex-col">
          ...
          <div className={`p-2 flex flex-wrap gap-1 justify-center bg-genshin-${element}/10 rounded-b-md border border-genshin-${element}/30`}>
            {chars.map(character => (
              <CharacterCard
                key={character.id}
                character={character}
